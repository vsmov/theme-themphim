...
<main class="pb-[32px] lg:pb-[100px] mt-[calc(var(--header-height)+16px)] lg:mt-[calc(var(--header-height)+52px)]">
    <section class="container">
        <div class="flex items-center justify-between max-[1132px]:justify-center">
            <h1 class="typography font-title text-[40px] leading-[100%] font-normal max-[1132px]:hidden">{{ $section_name }}</h1>
            @include('themes::themethemphim.inc.catalog_filter')
        </div>
    </section>
    <section class="lg:mt-16 container mt-8">
        <div>
            ...
            {{ $data->appends(request()->all())->links('themes::themethemphim.inc.pagination') }}
        </div>
    </section>
</main>
@endsection
